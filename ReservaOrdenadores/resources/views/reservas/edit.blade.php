            ...
            <div>
                @foreach ($reservasLista as $reserva)
                    @if ($reserva->idOrdenador == $ordenador->id)
                        ...
                        @else
                            <button class="bg-red-500 hover:bg-red-400 text-white font-bold py-2 px-4 border-b-4 border-red-700 hover:border-red-500 rounded" disabled>{{substr($reserva->jornada->hora->intervalo, 0, 5)}}</button>
                        @endif
                    @endif
                @endforeach
            </div>
